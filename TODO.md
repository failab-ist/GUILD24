...
| C04 | **Keyboard focus is lost whenever a redraw happens under an open modal.** `renderModal()` rebuilds `#modal-root` wholesale on every redraw, destroying the focused control inside it; focus falls to `<body>` while the modal is still open, so a keyboard user has to tab back from the top of the document. Reached by a real path — open the shop menu, tap sound on/off. Predates the v2.4 focus work and lives in a different owner (`#modal-root`, not `#app`), so it was measured and filed rather than fixed at the close. `npm run qa:visual` reports it as a warning on every run. |
...
- Remote CI (**C03**)
- Keyboard focus under an open modal (**C04**)
