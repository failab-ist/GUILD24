...
- v2.9.2 밸런스 검토(User 2026-09-25, 3개 레버만): 대성공 EXP 1.10 반영 완료. GATE SLOPE early 1.40 / 1.50과
  정가 구매의사 ×0.90 / ×0.80은 측정만 함 → `reports/v292-balance-review.md`. **User 결정 대기** (권고: slope 1.40 + 정가 ×0.90으로
  1차 플레이 테스트). 인간형: User 저장(2런 D1) 대조로 `human`을 주 렌즈로 채택(§2). 가격 모드 비율·사망 수 대조는 D30 직후 저장이 오면 한다.
