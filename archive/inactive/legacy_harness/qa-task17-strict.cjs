...
setTimeout(async () => {
  ...
  try {
    ...
    await page.fill('#seed', 'qa-final-strict');
    await page.click('[data-action="start"]');
    
    await page.waitForTimeout(300);
    if (await page.$('#modal-root [data-action="buy-relic"]')) {
    ...
    await page.waitForSelector('[data-action="contract"]');
    await page.click('[data-action="contract"]');
    await page.waitForTimeout(500);
    
    const preCheck = await page.evaluate(async () => {
      const g = window.Guild24.game;
      const r = g.run;
      if (r.phase !== 'sell') return { error: 'Phase is not sell' };
...
}, 3000); // Wait 3s before starting playwright
