// v2.9.2 balance measurement - the agreed Gate-count + Warehouse package, Level Death protection on vs off (User 2026-09-26).
...
// Paired D10 fork (as tools/measure-arms-v292.cjs): each seed is replayed through DAY 1~10 on Production, the D10 state is
// fingerprinted at DAY 11's morning (before its Gates are rolled), and only then the arm switches on for DAY 11~30. The
// fingerprint must match across arms for every seed. Arms:
//   base  Production (third pass: Gate D11~20 1.10, levelFactor floor .85, late T3 +0.10)
//   P     package: Gate count D19~24 3 Gates at 70% (2 at 30%, one draw), D25~29 exactly 3 (no draw); 후방 창고 증설 +10 -> +5 slots
//   PL    P + Level Death protection off (levelFactor = 1)
// A Warehouse bought before the fork (DAY 0 / 5 windows) runs at +10 through DAY 10 and at +5 from DAY 11 in P / PL; the rows
// record the purchase Day so that share is visible.
//   node tools/measure-package-v292.cjs <out.json> [seeds=5000] [arms=base,P,PL]      (PAR workers, default CPU count)
const fs=require('node:fs'),path=require('node:path'),os=require('node:os'),{spawn}=require('node:child_process');
const ARMS=['base','P','PL'];
// the levers each arm switches on at the fork
const LEVERS={base:[],P:['gates','wh'],PL:['gates','wh','lOff'],L:['lOff']};
if(process.argv[2]==='--worker'){
 ...
 patch("function levelFactor(level){return Math.max(.85,1-.015*((level||1)-1));}","function levelFactor(level){if(globalThis.__L_OFF)return 1;return Math.max(.85,1-.015*((level||1)-1));}");
 ...
 const sj=path.join(dir,'systems','shop.js');let u=fs.readFileSync(sj,'utf8');const gOld='count=counts.length===1?counts[0]:this.rng.int(counts[0],counts.at(-1));';
 if(!u.includes(gOld))throw Error('patch site moved: gate count');
 u=u.replace(gOld,"count=globalThis.__GATES&&s.day>=19&&s.day<=29?(s.day>=25?3:(this.rng.next()<.70?3:2)):counts.length===1?counts[0]:this.rng.int(counts[0],counts.at(-1));");fs.writeFileSync(sj,u);
 ...
 const whSlots=DATA.relicParams.warehouse.slots;
 const on=a=>{const L=LEVERS[a];if(!L)throw Error('unknown arm '+a);globalThis.__GATES=L.includes('gates');globalThis.__L_OFF=L.includes('lOff');
  DATA.relicParams.warehouse.slots=L.includes('wh')?5:whSlots;};
 ...
 globalThis.__DEATH_ROLL=(roll,unreduced,rolled,level,day)=>{if(!cur||!cur.fork)return;const lf=Math.max(.85,1-.015*((level||1)-1)),prod=unreduced*lf;
 ...
 P.start=function(seed,...x){on('base');const m=/^revision-(\d+)$/.exec(seed);const r=start.call(this,m?'revision-'+(Number(m[1])+off):seed,...x);
  cur={seed:Number(/^revision-(\d+)$/.exec(this.run.seed)?.[1]??-1),fork:null,lvlSaved:{},lvlExtra:{},t3:{n:0,worst:{}},at:{},gates:{a:[0,0],b:[0,0]},inv29:0};this.__cur=cur;return r;};
 ...
 const order=P.order,stock=P.stock;
 // peak stock after an ORDER buy, DAY 19~29 (the Warehouse cap binds here, not after the day's sales)
 const confirmOrder=P.confirmOrder;P.confirmOrder=function(...x){const r=confirmOrder.apply(this,x);const s=this.run,c=this.__cur;if(c&&c.fork&&s.day>=19&&s.day<=29)c.inv29=Math.max(c.inv29,s.inventory.length);return r;};
 P.order=function(...x){const r=order.apply(this,x);const s=this.run,c=this.__cur;if(c&&c.fork&&s.day>=19&&s.day<=29)c.inv29=Math.max(c.inv29,s.inventory.length);return r;};
 P.night=function(...x){const s=this.run,c=this.__cur;
  if(c&&c.fork&&s.day>=19&&s.day<=29){const g=s.day<=24?c.gates.a:c.gates.b;g[0]++;g[1]+=s.dungeons.length;}
 ...
 P.end=function(w,y){const s=this.run,c=this.__cur;const r=end.call(this,w,y);
  if(c&&c.fork){const R=s.npcs.flatMap(n=>n.records||[]),O={s2:{},s3:{}},T3={};
   ...
   rows.push({i:c.seed,fp:c.fork.fp,d10top3:c.fork.top3,d10deaths:c.fork.deaths,day:s.day,end:s.stats.deaths>=Meta.deathLimit(s)?'D':s.bossDebug?(s.win?'C':'F'):'B',
    ...
    wh:(s.relicHistory||[]).find(h=>h.purchased==='warehouse')?.purchaseDay??null,gates:c.gates,inv29:c.inv29,boss:s.bossDebug?{power:s.bossDebug.power,boss:s.bossDebug.bossPower}:null});}
...
const out=process.argv[2]||'reports/v292-package.json',seeds=Number(process.argv[3]||5000),arms=(process.argv[4]||ARMS.join(',')).split(','),
 PAR=Number(process.env.PAR||os.cpus().length),BLOCK=250,TMP=fs.mkdtempSync(path.join(os.tmpdir(),'g24-pkg-'));
...
(async()=>{
 ...
 const res={generated:'v2.9.2 Gate-count + Warehouse package, Level Death protection on / off, paired D10 fork',policy:'reader',account:'fresh',seeds:'revision-0..'+(seeds-1),arms,
 ...
 for(const [name,set] of Object.entries(cohorts)){const c={n:set.length,byArm:{}};
  for(const a of arms){const R=set.map(id=>by[a].get(id)).filter(Boolean),r20=R.filter(r=>r.day>=20),r29=R.filter(r=>r.day>=29);
   ...
   const x={reach20:share(R,r=>r.day>=20),reach30:share(R,r=>r.day>=30),d10to20:share(R,r=>r.day>=20),d20to30:share(r20,r=>r.day>=30),
    ...
    gatesPerDayD19_24:(()=>{const g=sum(R,r=>({d:r.gates.a[0],n:r.gates.a[1]}));return g.d?g.n/g.d:null;})(),gatesPerDayD25_29:(()=>{const g=sum(R,r=>({d:r.gates.b[0],n:r.gates.b[1]}));return g.d?g.n/g.d:null;})(),
    warehouseBought:share(R,r=>r.wh!==null),warehouseBoughtByD10:share(R,r=>r.wh!==null&&r.wh<=10),maxInventoryD19_29:med(r29.map(r=>r.inv29)),
    finalReached:R.filter(r=>r.boss).length,finalPowerOverBoss:med(R.filter(r=>r.boss).map(r=>r.boss.power/r.boss.boss)),finalSafeAtMinRoll:share(R.filter(r=>r.boss),r=>r.boss.power*.88>=r.boss.boss)};
