...
**노출 위치**
> Gate 상세(아침 게이트 판의 위험 보기) / 목적지 판 Help의 위험 행. 게이트 수치가 맨 앞. 아홉 위험 모두 같은 형식이며 이름만 있는 위험은 없다. 손님별 '더 필요' 수치는 쓰지 않는다.
...
**현재 — 아침 게이트 판 짧은 행** (User 2026-09-24 revision, v2.9.0)
> {위험} · 대응 {N} 필요 · {능력치} 10마다 대응 {k}

**압박 Label (능력치별, 짧은 행 형식 `{위험} · {label}`)**
> 강인함으로 버틴다  
> 기동으로 피한다  
> 정신으로 견딘다
