...
OWNER=presentation_polish,visual_detail,phase_material,game_like_interaction,audio_presentation,functional_presentation
DOC_VERSION=2.8.0
DESIGN_SSOT=GUILD24_DESIGN_SSOT_v2.8.0
DOC_AUTHORITY=AUTHORITATIVE_DESIGN_SPEC
SOURCE_SPLIT_FROM=UI_UX_v2.8.0
PATCH_TYPE=PRESENTATION_DETAILING

## AUTHORITY / PURPOSE

This document owns the v2.8 Player-facing Presentation Polish contract that was previously embedded
inside UI_UX_v2.8.0.md.

The split is organizational only: the pre-existing Presentation Polish rules below keep the same
authority. They are not duplicated in UI_UX_v2.8.0.md.

The active User direction for this pass is:

    KEEP THE APPROVED COMPOSITION
    -> UPGRADE THE DETAILING / FINISH
    -> DO NOT REDESIGN THE SCREEN JUST TO MAKE IT FEEL NEW

The Lust / 제0게이트 FINAL reference is a QUALITY BAR, not a skin to copy.

Take from that reference:
- deliberate edge construction
- controlled frame / separator weight
- tactile pixel controls
- clean alignment
- strong local hierarchy
- material-specific finish
- dense but intentional micro-detail
- a screen that looks authored rather than browser-default

Do NOT copy from that reference into unrelated Phases:
- blood / ember-red palette
- demon / gothic motifs
- jewel treatment
- ornamental corner language
- boss-specific frame identity

## PRESENTATION DETAILING DIRECTIVE — USER 2026-09-23

### 1. COMPOSITION LOCK IS THE DEFAULT

Presentation Upgrade does NOT mean re-layout by default.

For a surface whose current information order / major object placement / action placement is already
Canonical and functionally sound:

- keep the major composition
- keep the information order
- keep the current interaction structure
- keep the current modal / stage ownership
- keep the current Primary / Secondary action relationship
- keep current gameplay-visible information

Do not turn:
- one-column into two-column
- modal into full-screen
- row into cards
- cards into tabs
- existing panel order into a new flow

merely because a different composition might also look good.

A composition change is allowed only when:
- current Canonical already requires it, or
- runtime evidence proves the current composition itself fails the owned UX / responsive QA, or
- the User explicitly asks for structural redesign.
...
When polishing one surface, work in this order:

    A. hierarchy / contrast
    B. object silhouette
    C. edge construction
    D. material separation
    E. control depth / press
    F. typography placement
    G. micro-spacing / alignment
    H. only then decorative detail, if the object still needs it

Do not start by adding ornaments.

If a surface still feels like web UI, first ask whether the problem is:
- every object has the same rectangle
- every edge has the same weight
- pressable and read-only planes have the same depth
- text is vertically floating instead of seated on the material
- neighbouring materials are too close in value
- spacing is inconsistent
- a Primary Action has no stronger silhouette / depth

Fix those before adding any mark.

### 3. EDGE CONSTRUCTION

Pixel detail comes from intentional edges, not from drawing boxes around everything.

Ordinary information surface:
- one clear outer separation is enough
- one inner rule / seam may be used when it corresponds to real structure
- avoid border + inset border + bevel + drop shadow stacks
- no soft shadow / blur

Pressable control:
- flat face
- deliberate lit edge where useful
- one bottom/right hard depth
- press collapses or reverses that depth
- the press must be visible in shape / position, not colour only

Read-only plane:
- flatter than a control
- no bottom/right press depth
- if recessed, recess must read as display / slot / ledger field rather than a disabled button

Major existing object such as a receipt, order form, report, gate or notice board:
- may own a stronger frame because the object itself is the surface
- strengthen its existing material edge rather than wrapping a second decorative frame around it

### 4. CORNER / SILHOUETTE DISCIPLINE

Do not use browser-default rounded-card language for game-critical surfaces.

Prefer:
- square corners
- existing stepped / cut pixel corners
- deliberate asymmetric notches only when already part of that component family

Do not apply the FINAL control silhouette to every Phase.

A special silhouette is spent on:
- the Phase's Primary commitment control
- an existing object whose physical identity requires it
- a major reveal acknowledgement when already owned

Utility controls remain simple.

### 5. MATERIAL FINISH

Each Phase keeps its own material.

Material is created by:
- flat colour plane separation
- existing texture tokens / art
- edge value differences
- hard cast depth
- object-specific seams / rules
- typography seated on the object

Material is NOT created by:
- glossy gradient
- fake metal shine
- universal bevel
- random noise
- repeated badges / bolts / seals
- recolouring the same generic card

When two adjacent planes merge visually, separate them by value / edge / depth before adding another
container.

### 6. CONTROL FINISH

Primary controls must feel intentionally built.

For every Primary Action inspect:
- face height
- label centring
- left/right optical padding
- hard depth thickness
- press travel
- focus treatment
- disabled treatment
- relationship to the information immediately above it

The label must look seated inside the face rather than floating in a generic rectangle.

Peer controls:
- same geometry
- same depth language
- no one peer becomes "recommended" through accidental brighter fill / thicker frame

Secondary / back / defer:
- same game language
- lower footprint / contrast / depth
- never look like another Primary

Disabled:
- keep readable cause / label
- remove pressability
- do not communicate disabled state by whole-element opacity alone

### 7. TYPOGRAPHY FINISH

Use the shipped typography roles deliberately.

- sign / plate face: short identity, Phase label, object heading
- UI face: explanatory copy, effects, conditions, action labels
- display / LED face: actual display role only

Do not make a surface more "game-like" by putting every line in the pixel/sign face.

Refine:
- baseline alignment
- title-to-rule spacing
- label/value alignment
- line-height inside controls
- numeric alignment where values compare

Text must align to the physical object it belongs to.

### 8. SPACING / ALIGNMENT

Presentation polish must remove accidental spacing.

Prefer a consistent local rhythm. When current geometry allows, tune spacing in 4px-class increments
and reuse the same gap for the same relationship.

Audit:
- repeated left edges
- repeated title baselines
- equal peer-control heights
- equal inset where objects belong to one family
- accidental 1–3px drift caused by nested padding
- dead space created only because a wrapper is taller than its content

Do not make everything evenly spaced. More space may separate semantic groups; less space binds a
label to its value or an Action to the information it consumes.

### 9. DETAIL DENSITY

The target is not minimalism.

A major game object may feel rich, but every detail must belong to:
- material
- hierarchy
- state
- control feedback
...
If removing a decorative mark changes none of those, remove it.

The FINAL reference feels finished because many small choices agree with one another, not because it
has the highest number of ornaments.

### 10. RESPONSIVE DETAILING

Do not redesign one breakpoint into a different product unless Canonical requires it.

PHONE:
- preserve the existing vertical narrative
- use the available width confidently
- keep gutters intentional and consistent
- do not shrink controls into desktop miniatures
- avoid nested frames that consume phone width

DESKTOP:
- do not leave the polished phone surface floating as a tiny centre column inside a black viewport
- first expand the existing stage / material / object scale / spacing within the same composition
- use available width for breathing room and owned content width
- do not invent a new two-column information architecture merely to fill space

Responsive polish is primarily scale, width use, spacing, crop and object proportion unless the
current Canonical already defines a layout change.

### 11. CSS-FIRST IMPLEMENTATION
...
Default implementation path:
    ...
    -> grid / flex tuning
    -> pseudo-elements
    -> clip-path where the component already warrants a special silhouette
    -> hard shadow / inset plane
    -> existing textures
    -> responsive sizing

Minimal wrapper/class changes are allowed only where CSS cannot reliably target the existing
semantic object.

Do not add markup solely to create decorative layers that pseudo-elements can supply.

### 12. SCREENSHOT REVIEW STANDARD

Source inspection is not enough.

For every polished surface compare runtime screenshots at the owned phone and desktop breakpoints.

Ask:
- Does this look like the same approved screen, only more finished?
- Is the first visual anchor unchanged but stronger?
- Are edges deliberate instead of generic?
- Does the material read before the decoration?
- Does the Primary Action feel pressable?
- Did any new detail compete with gameplay information?
- Did desktop become larger / richer rather than simply wider and emptier?
- Did phone lose width to unnecessary nested frames?

If the answer to the first question is "this is a different composition", stop and justify that
change against Canonical before continuing.

## BATCH 1 — DETAILING TARGET

### OPENING / PRE-RUN

Keep:
- current title-card + preparation-modal composition
- current content order
- current Decoration Slot rows
- current Store Management path
- current Primary start Action

Detail:
- make title / branch identity read as authored signage rather than plain centred text
- tighten the relationship between modal header, preparation body and footer without changing their
  order
- turn Slot rows from faint web-list rows into crisp installation/readout rows using edge, baseline,
  state and spacing
- give the Primary start control the strongest physical finish in this surface
- keep Store Management clearly Secondary
- remove accidental mismatched edge weights between the modal, rows and Action

Do NOT:
- turn this into a new two-column desktop composition
- replace the modal with a new full-screen flow
- invent new preparation information

### STORE MANAGEMENT

Keep:
- Codex / management ownership
- current four Slot sections
- current option order
- current two-track option row
- current buy / equip / unequip behavior
- current inline purchase confirmation

Detail:
- make each Slot header read as a real workbench location heading
- make option rows share exact edge / seam logic
- keep purchase YELLOW as a local controlled pop only
- make equipped / unequipped / unavailable states read through construction as well as colour
- remove whole-card-opacity treatment where it weakens required copy or violates unavailable-state
  grammar
- align effect, price/state and Action faces precisely across sibling rows
- ensure the right-hand Action face feels physically integrated with the row rather than like a web
  button placed beside text

Do NOT:
- replace the four Slot sections with a new layout system
- add a collection grid / rarity UI / decorative slot frame system
...
- DAY sign
- notice board
- existing paper slips
- wall / counter / till
- shutter Action position

Detail:
- sharpen the notice board as one physical object using its existing frame / cork material
- refine paper-slip shadow, edge and internal spacing without adding extra frames
- seat pins / headings / hazard rows more precisely on the paper
- improve value separation between wall, board, slips, counter and dock so they read as one room
  with distinct materials
- polish till/readout alignment and the shutter pull's flat hard-depth construction
- use the existing room art / crop / CSS planes to make wide screens feel intentionally filled

Do NOT:
- add more signs, bolts, brackets or decorative shop props merely to fill space
- add a new board layer or frame around the whole screen
- change the Morning information order
