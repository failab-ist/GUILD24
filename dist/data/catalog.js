...
item('rice','삼각김밥',0,35,70,'food',2,'rice','용사픽','김 끝을 잡고 천천히.',{survival:6,supply:4}),
item('water','생수',0,40,85,'drink',2,'water','용사픽','뚜껑까지 챙겨 돌아오세요.',{survival:10,supply:2}),
item('ramen','컵라면',0,45,90,'food',3,'ramen','원정한끼','뜨거운 국물과 약간의 냉기 저항.',{cold:10,supply:2}),
item('bar','간단 도시락',1,85,180,'food',2,'bar','용사픽','반찬은 단출하지만 빈칸은 없다.',{survival:10,supply:5,loot:0.2}),
item('choco','초코바',0,30,65,'food',2,'choco','용사픽','주머니에서 녹기 전에 드세요.',{mobility:8,supply:3}),
item('coffee','캔커피',0,40,85,'drink',2,'coffee','MANA+','발걸음이 조금 가벼워진다.',{mobility:12,supply:2}),
/* Replaces the retired 붕대 slot as a plain Spirit route - not a fear/dark/whiteout Counter. */
item('herbtea','진정 허브티',0,40,85,'drink',2,'herbtea','MANA+','마시기 전에 심호흡부터 하는 손님이 많다.',{spirit:15,supply:2}),
item('potion','하급 포션',0,70,140,'potion',3,'potion','귀환안심','차갑게 보관하지 않아도 됩니다.',{combat:8,potion:1}),
item('ice','얼음컵',0,30,65,'drink',3,'ice','용사픽','컵에 얼음만 가득 담아 판다. 녹기 전에 도착하길.',{fire:10,supply:1}),
item('battery','랜턴 건전지',0,45,95,'gear',3,'battery','귀환안심','흔들면 조금 더 간다. 근거는 없다.',{dark:16}),
item('rope','경량 로프',0,50,105,'gear',3,'rope','귀환안심','매듭을 풀고 감았다. 다시 묶어야 한다.',{bind:16}),
item('candy','집중 사탕',0,35,75,'food',4,'candy','용사픽','시험 전에도 잘 팔린다.',{fear:10,supply:2}),
item('lava','불룡볶음면',1,65,135,'food',3,'ramen','원정한끼','용 그림은 장식이 아니다.',{survival:5,cold:6,supply:2}),
item('energy','에너지드링크',1,70,150,'drink',3,'energy','MANA+','오늘 쓸 기운을 당겨왔다.',{mobility:15,supply:2}),
item('wine','용사의 곡주',1,60,130,'drink',4,'wine','원정한끼','공포를 잊게 한다. 발걸음은 살짝 꼬인다.',{fear:18,mobility:-4,supply:1}),
...
item('kit','구급키트',1,80,170,'insurance',4,'kit','귀환안심','열어 본 사람은 대개 그날을 오래 기억한다.',{aftercare:1}),
item('mask','방진마스크',1,65,135,'gear',4,'mask','귀환안심','쓰고 나면 얼굴 자국이 한참 남는다.',{poison:12}),
item('heat','핫팩',1,60,130,'gear',4,'heat','귀환안심','주머니 안에서 겨울을 버틴다.',{cold:18}),
item('cloak','방수망토',1,75,160,'gear',4,'cloak','귀환안심','부식과 진창에 두루 쓴다. 어느 쪽도 전문가만은 못하다.',{corrosion:6,mire:6}),
item('coating','부식 방지 코팅제',1,70,150,'gear',4,'coating','귀환안심','장비 겉면에 얇게 펴 바른다. 굳기 전에 서두를 것.',{corrosion:18}),
item('boots','원정용 장화',1,65,135,'gear',4,'boots','귀환안심','밑창에 진흙이 잘 붙지 않는다.',{mire:16}),
item('snowgoggles','설원 고글',1,60,125,'gear',4,'goggles','귀환안심','눈보라 속에서도 앞이 남는다.',{whiteout:16}),
item('highpotion','상급 포션',2,150,300,'potion',5,'potionHigh','길드초이스','작은 병에 진하게 담았다.',{combat:16,potion:1}),
/* Dedicated Poison specialist only: no generic Core Stat, and no poison cure subsystem. */
item('antidote','농축 해독제',2,80,170,'gear',5,'antidote','귀환안심','한 모금이면 충분하다고 적혀 있다. 두 모금은 권하지 않는다.',{poison:18}),
...
item('midpotion','중급 포션',1,110,230,'potion',4,'potionMid','귀환안심','하급은 불안하고 상급은 비쌀 때.',{combat:12,potion:1}),
item('premium','길드 특제 도시락',2,160,340,'food',2,'lunch','길드초이스','뚜껑이 잘 안 닫힌다.',{survival:14,supply:6,loot:0.4}),
...
item('spiderkit','거미줄 방호세트',3,150,320,'gear',5,'spiderkit','귀환안심','손목을 앞으로 내밀어도 아무것도 나오진 않는다.',{poison:12,bind:12}),
item('slimesuit','연금 방수슈트',3,150,320,'gear',5,'slimesuit','귀환안심','방수 테스트에 쓴 액체는 묻지 않는 게 좋다.',{corrosion:12,mire:12}),
item('cryptlantern','성화 랜턴',3,150,320,'gear',5,'cryptlantern','귀환안심','성당 납품용이었는데 어쩌다 편의점까지 왔다.',{fear:12,dark:12}),
item('snowvisor','백설 방한고글',3,150,320,'gear',5,'snowvisor','귀환안심','김은 안 서린다. 눈썹은 얼 수 있다.',{cold:12,whiteout:12}),
item('magmagear','마그마 냉각장비',3,160,340,'gear',5,'magmagear','귀환안심','설명서 첫 줄: 마그마에 직접 넣지 마시오.',{fire:14,combat:6}),
...
item('herobar','왕도 천연암반수',3,185,390,'drink',3,'herobar','길드초이스','왕도 외곽 암반층에서 길어 올렸다고 적혀 있다.',{survival:20,supply:2}),
item('hyperenergy','초고속 에너지드링크',3,160,340,'drink',3,'hyperenergy','MANA+','마시고 나면 계산대보다 먼저 문을 나선다.',{mobility:18,supply:2}),
item('sageelixir','대현자 허브엘릭서',3,160,340,'drink',3,'sageelixir','길드초이스','한 모금 마시면 괜히 턱을 쓰다듬게 된다.',{spirit:20,supply:2}),
item('toppotion','최상급 포션',3,190,400,'potion',5,'toppotion','길드초이스','병은 작다. 값은 작지 않다.',{combat:24,potion:1})
],
