...
['showcase','희귀상품 입고 계약','foundation',['premium'],280,'희귀 이상 발주 가중치 +70%. 다음 날부터 운영비 +10G.'],
...
['fridge','대형 냉장고','foundation',['fresh'],200,'음식·음료 유통기한 +1일. 보유 중인 해당 재고도 획득 시 한 번 연장.'],
['kitchen','즉석식품 코너','foundation',['fresh'],280,'음식·음료가 원래 가지는 능력치 증가 효과 +30% · 보급·위험 대응·특수용 제외.'],
['board','길드 전광판','foundation',['customer'],260,'기본 방문객이 3명인 날 4명으로 올린다.'],
['rookieBoard','신입 모집 게시판','foundation',['customer'],240,'신규 모험가가 생기는 날, 새 모험가가 오늘 방문객 1명으로 반드시 등장 · 총 방문객 수는 변함 없음.'],
['groupFlyer','공동구매 전단','hybrid',['rotation','customer'],400,'오늘 방문객 6명 이상이면 같은 상품 3개 이상 발주 시 매입가 -10%.'],
['memberBundle','단골 묶음혜택','hybrid',['rotation','vip'],380,'재방문 손님의 오늘 두 번째 유료 구매 시 단골도 +2.'],
...
['expeditionMeal','원정 도시락 코너','hybrid',['fresh','expedition'],400,'실제 목적지에 맞는 음식·음료의 위험 대응 +25% · 보급이 필요한 날, 보급을 주는 음식·음료의 원래 능력치 증가 효과 +20%.'],
...
['lifetime','평생 단골제','keystone',['vip'],740,'단골도 60 이상 손님이 생환하면 하루 1번 소지금 +50G · 다음 방문 가중치 +50%.'],
['royalCert','왕도 프리미엄 인증','keystone',['premium'],760,'희귀 이상 상품을 150%로 판매하면 정가의 20% 본사 수당.'],
...
['fresh24','24시간 신선체계','keystone',['fresh'],740,'음식·음료 유통기한 +2일 · 원래 가지는 능력치 증가 효과 +50% · 보급·위험 대응·특수용 제외.'],
['hub','지역 거점점 계약','keystone',['customer'],700,'다음 아침 방문객 +1명 45% · +2명 15% · 증가 없음 40% · 기본 운영비 +10%.'],
...
D.relics=rows.map(([id,name,kind,tags,price,description])=>({id,name,kind,tags,price,description,
 ...(id==='guarantee'?{minPrice:GUARANTEE_MIN_PRICE}:{}),finalUseful: !['stamp','member','guarantee','fridge','board','rookieBoard','groupFlyer','memberBundle','premiumMember','returnPoints','supplyCert','lifetime','royalCert','hub','efficiency'].includes(id)}));
