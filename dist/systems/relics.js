...
w.purchaseDay=s.phase==='foundation'?0:s.day;
 const extension=id==='fridge'?1:id==='fresh24'?2:id==='coldcase'?1:0;if(extension)for(const st of s.inventory){const it=D.itemBy[st.item];if(st.expires!==null&&st.expires>s.day&&food(it)&&(id!=='coldcase'||it.rarity>=1)){st.extensions??=[];if(!st.extensions.includes(id)){st.expires+=extension;st.extensions.push(id);}}}
