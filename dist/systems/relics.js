...
w.purchaseDay=s.phase==='foundation'?0:s.day;
 const extension=['fridge','fresh24','coldcase'].includes(id)?D.relicParams[id].shelfDays:0;if(extension)for(const st of s.inventory){const it=D.itemBy[st.item];if(st.expires!==null&&st.expires>s.day&&food(it)&&(id!=='coldcase'||it.rarity>=1)){st.extensions??=[];if(!st.extensions.includes(id)){st.expires+=extension;st.extensions.push(id);}}}
...
P.relicQuote=function(index,quantity,cart=this.run.cart||{}){const s=this.run,o=s.offers[index],it=D.itemBy[o.item];const entries=Object.keys(cart),skuTotal=entries.filter(i=>s.offers[i].item===o.item).reduce((n,i)=>n+cart[i],0),prior=entries.filter(i=>Number(i)<Number(index)&&s.offers[i].item===o.item).reduce((n,i)=>n+cart[i],0),firstSKU=entries.map(i=>s.offers[i].item).find(id=>entries.filter(i=>s.offers[i].item===id).reduce((n,i)=>n+cart[i],0)>=3),first=o.item===firstSKU;let sum=0;for(let unit=1;unit<=quantity;unit++){let mult=1;if(skuTotal>=3){if(this.has('bulk')&&prior+unit>=3)mult*=1-D.relicParams.bulk.discount;if(this.has('groupFlyer')&&s.queue.length>=6)mult*=1-D.relicParams.groupFlyer.discount;if(food(it)&&this.has('dawnBulk'))mult*=1-D.relicParams.dawnBulk.discount;if(!s.bulkUsed&&first&&s.previousSales>=7&&this.has('logisticsHQ'))mult*=1-D.relicParams.logisticsHQ.discount;}sum+=Math.round(o.price*Math.max(.45,mult));}return sum;};
...
G.Relics={food,field,known,counter,offerWeight(g,it){let w=1;const has=id=>g.has(id);if(has('showcase')&&it.rarity>=2)w*=D.relicParams.showcase.rareWeightMult;if(has('hazardBoard')&&counter(it,known(g)))w*=D.relicParams.hazardBoard.weightMult;if(has('medicine')&&field(it))w*=D.relicParams.medicine.weightMult;if(has('coldcase')&&food(it)&&it.rarity>=1)w*=D.relicParams.coldcase.weightMult;return w;},shelf(g,it){if(!food(it))return 0;const p=D.relicParams;return (g.has('fridge')?p.fridge.shelfDays:0)+(g.has('fresh24')?p.fresh24.shelfDays:0)+(g.has('coldcase')&&it.rarity>=1?p.coldcase.shelfDays:0);}};
})(globalThis);
