...
P.relicQuote=function(index,quantity,cart=this.run.cart||{}){const s=this.run,o=s.offers[index],it=D.itemBy[o.item];const entries=Object.keys(cart),skuTotal=entries.filter(i=>s.offers[i].item===o.item).reduce((n,i)=>n+cart[i],0),prior=entries.filter(i=>Number(i)<Number(index)&&s.offers[i].item===o.item).reduce((n,i)=>n+cart[i],0);let sum=0;for(let unit=1;unit<=quantity;unit++){let mult=1;if(skuTotal>=3){if(this.has('bulk')&&prior+unit>=3)mult*=1-D.relicParams.bulk.discount;if(this.has('groupFlyer')&&s.queue.length>=6)mult*=1-D.relicParams.groupFlyer.discount;if(food(it)&&this.has('dawnBulk'))mult*=1-D.relicParams.dawnBulk.discount;if(s.previousSales>=6&&this.has('logisticsHQ'))mult*=1-D.relicParams.logisticsHQ.discount;}sum+=Math.round(o.price*Math.max(.45,mult));}return sum;};
