...
G.Relics={food,field,known,counter,offerWeight(g,it){let w=1;const has=id=>g.has(id);if(has('showcase')&&it.rarity>=2)w*=D.relicParams.showcase.rareWeightMult;if(has('hazardBoard')&&counter(it,known(g)))w*=D.relicParams.hazardBoard.weightMult;if(has('coldcase')&&food(it)&&it.rarity>=1)w*=D.relicParams.coldcase.weightMult;return w;},shelf(g,it){if(!food(it))return 0;const p=D.relicParams;return (g.has('fridge')?p.fridge.shelfDays:0)+(g.has('coldcase')&&it.rarity>=1?p.coldcase.shelfDays:0);}};
})(globalThis);
