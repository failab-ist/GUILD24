...
P.specialAction=function(npcId,value){const s=this.run,e=s.special,n=s.npcs.find(n=>n.id===npcId);if(!e||e.used||!n?.alive||!n.introduced)throw Error('선택 가능한 모험가가 아닙니다.');if(e.kind==='route'){if(s.phase!=='sell'||this.current()?.id!==npcId||n.pack.length||n.history.some(h=>h.day===s.day))throw Error('아직 거래하지 않은 현재 손님만 배치 조정할 수 있습니다.');const index=Number(value);if(!Number.isInteger(index)||!s.dungeons[index]||index===n.destination)throw Error('다른 게이트를 선택해 주세요.');n.destination=index;n.claimedDestination=index;}else{if(!['morning','order'].includes(s.phase))throw Error('영업 준비 중에 선택할 수 있습니다.');if(e.kind==='remove'){if(!n.traits.includes(value)||D.traitBy[value]?.direction!=='negative')throw Error('제거할 약점을 선택해 주세요.');n.traits=n.traits.filter(t=>t!==value);}else{if(!e.candidates.includes(value)||n.traits.includes(value)||n.traits.length>=Math.min(4,n.traitSlots)||D.traitExclusions.some(pair=>pair.includes(value)&&pair.some(t=>n.traits.includes(t))))throw Error('이 특성을 배울 수 없습니다.');n.traits.push(value);}}e.used=true;s.specialUsed=true;s.notice='길드 지원을 받았습니다.';this.save();};
