...
function renderModal(){const root=$('#modal-root');if(!modal){root.innerHTML='';document.body.style.overflow='';return;}
 ...
 if(modal==='boss'){const c=Copy.boss,stage=bossRevealStage();
  ...
  narrow=stage!=='d30';}
 else if(modal==='new'){title='새 점포 준비';body=(Save.error?'<p class="save-alert">'+E(Save.error)+'</p>':'')+newRun();footer=game.run&&game.run.phase!=='end'?btn('현재 런 포기 · 새 점포 준비','start','danger'):btn('첫 점포지원 고르기','start','stamp');narrow=true;}
 ...
 else if(modal==='resetConfirm'){title='게임 데이터 초기화';body='<p>이 브라우저에 보관된 GUILD24 저장을 전부 지웁니다. 현재 영업, 모험가, 재고, 자금, 점포지원과 함께 직업 숙련 · 마왕 토벌 기록 · 가맹등급 · 몬스터 지식 · 해금과 점주 가이드 진행까지 남지 않습니다.</p><p class="danger-text">되돌릴 수 없습니다. 남기고 싶다면 먼저 저장을 내보내 주세요.</p>';footer=btn('저장 내보내기','export')+btn('취소','dismiss')+btn('전부 지우기','reset-go','danger');narrow=true;}
