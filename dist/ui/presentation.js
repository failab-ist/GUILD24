(function(G){
const D=G.DATA;
const labels={supply:'보급',whiteout:'화이트아웃 대응',combat:'전투',survival:'강인함',mobility:'기동',spirit:'정신',cold:'냉기 저항',fire:'화염 저항',poison:'독 저항',fear:'공포 저항',dark:'어둠 대응',undead:'언데드 상대 전투',long:'장기 원정 대응',wet:'습지 대응',corrosion:'부식 대응',bind:'속박 대응',slow:'기동 감소 대응',armor:'방어 대응',food:'포만감',thirst:'갈증',fatigue:'누적 피로',escape:'탈출 보정',injuryGuard:'부상 방어',injuryRisk:'부상 위험',loot:'전리품',xpMult:'경험치',luck:'행운 보정',variance:'판정 변동폭',rareLoot:'장비 획득 보정',foodMult:'음식 포만감·보급·강인함',defenseMult:'상품 강인함 효과',healMult:'상품 강인함 효과',potionMult:'포션 강인함 효과',caffeineMult:'카페인 상품 기동 효과',alcoholMult:'곡주 공포 저항 효과',priceBias:'120G 초과 구매 의사',buyBias:'구매 의사'};
const percent=new Set(['escape','injuryGuard','injuryRisk','loot','luck','variance','rareLoot','priceBias','buyBias']);
const mult=new Set(['xpMult','foodMult','defenseMult','healMult','potionMult','caffeineMult','alcoholMult']);
const negative=new Set(['thirst','fatigue','injuryRisk','variance']);
const util={duplicate:'다음 소비품 효과 2회 적용 · 쿠폰도 1칸 사용 · 중첩 불가',revive:'사망 판정을 중상으로 변경',curePoison:'독 대응 상품',potion:'포션',caffeine:'카페인 함유',alcohol:'곡주 · 기동 저하 동반'};
...
function traitText(id){const t=D.traitBy[id];let text=rows(t.effects).map(r=>r.label+' '+r.text).join(' · ');if(id==='showoff')text='말한 목적지가 실제와 다를 수 있음 · 실제 배정은 바뀌지 않음';if(id==='eater')text+=' · 음식 구매 의사 +12%p';if(id==='caffeine')text+=' · 카페인 상품 구매 의사 +20%p';if(id==='drinker')text+=' · 곡주 기동 페널티 70% 완화';return text;}
