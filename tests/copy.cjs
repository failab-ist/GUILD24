...
test('COPY_AUDIT §11: all 30 Store Support names / prices / descriptions are the approved text',()=>{
 const SUPPORTS=[
  ...
  ['guarantee','길드 보증 진열대',200,'하루 1회 · 정가 200G 이상 상품 첫 판매 시 본사가 정가의 20%를 손님 대신 부담. 점주는 선택한 판매가 전액 수령.'],
  ['hazardBoard','원정 위험 게시판',120,'오늘 게이트의 위험에 대응하는 상품이 발주 후보에 나올 가중치 +50%.'],
  ['medicine','긴급보급 선반',150,'포션·야외장비·보험 발주 가중치 +60% · 해당 상품 공급 수량 +1.'],
  ['fridge','대형 냉장고',120,'음식·음료 유통기한 +2일 · 처음 확보할 때 보유 중인 해당 재고도 1회 연장.'],
  ['kitchen','즉석식품 코너',240,'음식·음료가 원래 가진 능력치 증가 효과 +25% · 보급·위험 대응·부작용 제외.'],
  ['board','길드 전광판',150,'기본 방문객이 3명인 날 4명으로 올린다.'],
  ['rookieBoard','신입 모집 게시판',150,'신규 모험가가 생긴 날, 그 모험가가 오늘 방문객 중 1명으로 반드시 등장 · 총 방문객 수는 늘지 않음.'],
  ['groupFlyer','공동구매 전단',280,'오늘 방문객 6명 이상이면 같은 상품 3개 이상 발주 시 매입가 -10%.'],
  ['memberBundle','단골 묶음혜택',270,'재방문 손님의 오늘 두 번째 유료 구매에 단골도 +2.'],
  ['premiumMember','프리미엄 멤버십',290,'단골도 50 이상 손님의 희귀 이상 상품 구매 의사 +10%p.'],
  ['returnPoints','귀환 적립제',340,'오늘 유료 구매한 재방문 손님이 생환하면 단골도 +5 · 소지금 +30G.'],
  ['expeditionMeal','원정 도시락 코너',280,'실제 목적지와 맞는 음식·음료의 위험 대응 +25% · 보급이 필요한 날, 보급을 주는 음식·음료의 원래 능력치 증가 효과 +20%.'],
  ['coldcase','냉장 유통 계약',250,'고급 이상 음식·음료 발주 가중치 +80% · 유통기한 +1일 · 처음 확보할 때 보유 중인 해당 재고도 1회 연장.'],
  ['supplyCert','길드 납품 인증',310,'현재 알려진 위험에 맞는 희귀 이상 상품 또는 희귀 이상 보험 판매 시 정가의 12% 본사 수당.'],
  ['dawnBulk','새벽 공동배송',270,'같은 음식·음료 3개 이상 발주 시 매입가 -15%.'],
  ['logisticsHQ','물류 본부계약',430,'전날 7건 이상 판매하면 다음 날 첫 묶음발주 매입가 -25%.'],
  ['lifetime','평생 단골제',440,'단골 손님이 생환하면 하루 1회 소지금 +50G · 다음 방문 가중치 +50%.'],
  ['royalCert','왕도 프리미엄 인증',460,'희귀 이상 상품을 150%에 판매하면 정가의 20% 본사 수당.'],
  ['expeditionCert','길드24 원정전문점 인증',420,'알려진 위험 1종이면 대응 상품 후보 1칸 보장 · 2종 이상이면 서로 다른 위험 2종의 대응 상품을 2칸 보장 · 후보를 교환해도 유지.'],
  ['fresh24','24시간 신선체계',520,'음식·음료 유통기한 +2일 · 원래 가진 능력치 증가 효과 +50% · 보급·위험 대응·부작용 제외.'],
