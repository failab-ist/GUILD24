...
test('fatigue persists as an NPC condition; no active Item alters it',()=>{const g=fresh(),base={...copy(g.run.npcs[0]),fatigue:10,traits:[],pack:[]},d=g.run.dungeons[0],a=copy(base),b={...copy(base),pack:['water']};Dungeon.resolve(a,d,new RNG('fatigue'));Dungeon.resolve(b,d,new RNG('fatigue'));/* DUNGEON_HAZARD v2.7 §FATIGUE OUTCOME BASELINE: 부상 is +6, not the superseded v2.6.1 +4. */assert.equal(a.fatigue,16);assert.ok(b.fatigue < a.fatigue, 'supply from water reduces fatigue');const rb=b.records[b.records.length-1];assert.equal(rb.preRecovery,3,'leftover Supply removes departure Fatigue 1:1');assert.equal(rb.remainingSupplyBuffer,0,'Supply spent on preRecovery cannot also buffer the result');assert.equal(rb.finalFatigue,rb.fatigueBeforeExpedition+rb.actualOutcomeFatigueGain,'final Fatigue is departure Fatigue plus the actual gain');});
