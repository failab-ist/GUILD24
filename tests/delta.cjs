...
test('food traits/relics enhance nutrition without amplifying counters or side effects',()=>{const g=fresh(),n={...g.run.npcs[0],traits:[],pack:['lava']},d=g.run.dungeons[0],base=Dungeon.prepare(n,d).effects,boost=Dungeon.prepare({...n,traits:['eater']},d,['kitchen','fresh24']).effects;assert.equal(boost.cold,base.cold,'Hazard Counter is not amplified');assert.equal(boost.combat,base.combat,'unrelated Stat is not amplified');assert.equal(boost.thirst,undefined,'no legacy thirst key survives');
 ...
 assert.ok(Math.abs(itemSurvival(['eater'],['kitchen','fresh24'])/bare-2.05)<1e-9,'대식가 +30% and the two Fresh Relics (+25% / +50%) add to base x2.05');
 assert.ok(Math.abs(itemSurvival([],['kitchen','fresh24'])/bare-1.75)<1e-9,'the two Fresh Relics alone are base x1.75');
 assert.ok(Math.abs(itemSurvival(['small'],['kitchen','fresh24'])/bare-1.55)<1e-9,'소식가 -20% subtracts from the same pool');
...
assert.ok(DATA.itemBy.heat.effects.cold>DATA.itemBy.lava.effects.cold);assert.equal(DATA.itemBy.tree.category,'insurance');assert.equal(DATA.itemBy.tree.rarity,3);assert.equal(DATA.itemBy.midpotion.effects.jobBonus,undefined);
/* SA-Q05: `potion` is an internal marker, never a player-facing effect row - the one key
   `Presentation.rows()` deliberately leaves unauthored, so it is exempted here too. */
for(const it of DATA.items){assert.ok(DATA.categories[it.category]);assert.ok(it.roles.length);for(const key of Object.keys(it.effects))assert.ok(key==='potion'||Presentation.rows(it.effects).some(r=>r.key===key));}});
test('canonical full-offer reroll replaces every slot, preserves pity and legality',()=>{const g=fresh();g.beginOrder();g.run.facilities=[];/* the old 원정전문점 Counter-slot guarantee left with the 2026-09-23 remake (원정 전문 인증) */const before=g.run.offers.slice(),pity=copy(g.run.pity),gold=g.run.money;g.reroll();assert.equal(g.run.offers.length,before.length);assert.ok(g.run.offers.every(o=>!before.includes(o)));assert.deepEqual(g.run.pity,pity);assert.equal(gold-g.run.money,DATA.balance.rerollBase);assert.ok(g.run.offers.every(o=>!DATA.itemBy[o.item].unlock||g.account.unlocked.includes(DATA.itemBy[o.item].unlock)));g.setQuantity(0,1);g.reroll();assert.equal(Object.keys(g.run.cart||{}).length,0);});
