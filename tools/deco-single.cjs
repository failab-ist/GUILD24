...
//   node tools/deco-single.cjs [seeds=1000] [--policy balanced] [--aware] [--set '{"memorialBoard":{"deathLimitBonus":1}}'] [--out file]
...
if(process.env.DECO_WORKER){
 process.on('message',({id,N,policy,aware,set})=>{const G=load();if(set)for(const [k,v] of Object.entries(set))Object.assign(G.DATA.decorationParams[k]??={},v);
  const a=G.Meta.fresh();if(id!=='none'){G.Meta.addCapital(a,G.DATA.decorationBy[id].price);G.Meta.buyDecoration(a,id);}
 ...
 const N=Number(args.find((a,i)=>/^\d+$/.test(a)&&!['--policy','--set','--out'].includes(args[i-1])))||1000,policy=flag('--policy')||'balanced',aware=args.includes('--aware');
 const set=flag('--set')?JSON.parse(flag('--set')):null;
 const G0=load(),ids=['none',...G0.DATA.decorations.map(d=>d.id)],res={};let live=0;const q=ids.slice();
