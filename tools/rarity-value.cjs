...
//   node tools/rarity-value.cjs [runs=600] [--potential 0.10] [--out file]   (--potential: the growth step every arm uses)
...
function load(p){for(const f of FILES){let src=fs.readFileSync(path.join(root,'dist',f+'.js'),'utf8');
  if(f==='systems/adventurer'){
   ...
   if(p.potential&&!p.benefitOff)src=one(src,'potential=1+rarity*.06+r.next()*.10','potential=1+rarity*'+p.potential+'+r.next()*.10');
...
if(process.env.RARITY_WORKER){
 ...
 const args=process.argv.slice(2),POT=args.indexOf('--potential')>=0?Number(args[args.indexOf('--potential')+1]):null,N=Number(args.find(a=>/^\d+$/.test(a))||600),oi=args.indexOf('--out'),res={};let live=0;const q=ARMS.slice();
 const next=()=>{if(!q.length){if(!live)done();return;}const [label,p]=q.shift();live++;
  const c=fork(__filename,[],{env:{...process.env,RARITY_WORKER:'1'}});c.on('message',m=>{res[m.label]=m;});c.on('exit',()=>{live--;next();});c.send({label,p:{...p,potential:POT},N});};
