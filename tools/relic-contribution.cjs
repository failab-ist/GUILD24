...
if(process.env.RELIC_WORKER){
 process.on('message',({seeds,id,overrides})=>{
  const G=load();
 ...
 const ids=pos[1]&&pos[1]!=='all'?pos[1].split(','):all;
 ...
 const next=()=>{if(!queue.length){if(!live)done();return;}const id=queue.shift();live++;
  ...
  c.on('message',({id,base,treat})=>{results[id]={gain:stat(base,treat,'gain'),reached:stat(base,treat,'reached'),win:stat(base,treat,'win')};
  ...
  c.on('exit',()=>{live--;next();});c.send({seeds,id,overrides});};
